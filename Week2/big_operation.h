...
public:
	...
	big operator + (const big &x) {
		...
		int num1 = 0;
		int num2 = ans.v2.size();
		if (num1 < num2) {
			while (num1 < ans.v2.size() && ans.v2[num1] == 0) {
				ans.v2.erase(ans.v2.begin());
			}
		}
		ans.count = ans.v2.size();
		ans.count1 = ans.v2.size();
		return ans;
	}
	...
	big operator - (const big &x) {
		...
		if (flag != x.flag) {//符号不一致时，两数绝对值相加，符号位与被减数一致
			flag = true;
			ans = (*this) + x;
			ans.flag = T;//无论绝对值大小，答案符号位与被减数一致
			return ans;
		}
		//同号时
	...
	big operator / (const big &x) {
		...
		if (beichushu >= chushu) {//如果被除数比除数大，就是有整数部分，先进行整数部分的计算
			...
			while (m > -2)
			{
				int num11 = beichushu.v.size() - 1;
				if (beichushu.v[num11] == 0 && num11 > 0) {
	...
	big operator * (const big &x) {
		big ans;
		int m = v.size(), n = x.v.size(), i, j;//乘法运算先将两个数的所有位数都放到一个队列里，最后在计算小数位数，方便处理
	...
	bool operator ==(const big &x) {
		big x2 = *this;
		int num1 = x2.v.size() - 1;
		big x1 = x;
		while (x2.v[num1] == 0 && num1 > 0) {
			x2.v.erase(x2.v.end() - 1);
			num1--;
		}
		num1 = x2.v2.size() - 1;
		if (num1 > -1) {
			while (num1 > -1&&x2.v2[num1] == 0 ) {
				x2.v2.erase(x2.v2.end() - 1);
				num1--;
			}
		}
		int num2 = x1.v.size() - 1;
		while (x1.v[num2] == 0 && num2 > 0) {
			x1.v.erase(x.v.end() - 1);
			num2--;
		}
		num2 = x1.v2.size() - 1;
		if (num2 > -1) {
			while (x1.v2[num2] == 0 && num2 > 0) {
				x1.v2.erase(x1.v2.end() - 1);
				num2--;
			}
		}
		//count = v2.size();
		if (x2.v.size() > x1.v.size()) {
			return 0;
		}
		if (x2.v.size() < x1.v.size()) {
			return 0;
		...
		if (x2.v2.size() > x1.v2.size()) {
			return 0;
		}
		if (x2.v2.size() < x1.v2.size()) {
			return 0;
		}
		for (int i = 0; i < x2.v2.size(); i++) {
			if (x2.v2[i] > x1.v2[i]) {
				return 0;
			}
			if (x2.v2[i] < x1.v2[i]) {
