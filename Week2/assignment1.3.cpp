...
double calculate_operator(double num1, double num2, char ch1) {//这个是传入数字来进行计算
	if (ch1 == '+') {
		return num1 + num2;
	}
	else if (ch1 == '-') {
...
double calculate(string str) {
	...
	while (n < str.length()) {
		if (str.at(n) >= '0'&&str.at(n) <= '9') {//处理数字，用atof把字符转化成数字
			...
			n++;
			numbers.push(argumentMap[str.at(n - 1)]);
			T = false;
			continue;
		}
			...
			numbers.push(testFuncString2(str0));
			n++;
		}
		else {//进行运算符的处理
			if (op.empty()) {
